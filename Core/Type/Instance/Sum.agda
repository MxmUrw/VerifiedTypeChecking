...
split-+-Str : ∀{A : 𝒰 𝑖} {B : 𝒰 𝑗} -> (x : A +-𝒰 B) -> (∑ λ (a : A) -> x ≡-Str left a) +-𝒰 (∑ λ b -> x ≡-Str right b)
split-+-Str (left x) = left (x , refl)
split-+-Str (just x) = right (x , refl)

split-+ : ∀{A : 𝒰 𝑖} {B : 𝒰 𝑗} -> (x : A +-𝒰 B) -> (∑ λ (a : A) -> x ≡ left a) +-𝒰 (∑ λ b -> x ≡ right b)
split-+ (left x) = left (x , refl)
split-+ (just x) = right (x , refl)

cancel-right : ∀{A B : 𝒰 ℓ} -> (b : B) -> (A +-𝒰 B) -> B
cancel-right b (left x) = b
cancel-right b (right x) = x

cancel-left : ∀{A B : 𝒰 ℓ} -> (a : A) -> (A +-𝒰 B) -> A
