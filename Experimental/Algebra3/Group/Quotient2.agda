...
module Verification.Experimental.Algebra2.Group.Quotient2 where
...
module _ {G} {{_ : Group 𝑗 on G}} where
  record isNormal (H : 𝒫 G) {{_ : Subgroup ′ G ′ on H}} : 𝒰 𝑗 where
...
module _ where
  module _ {G H} {{_ : Group 𝑗 on G}} {{_ : (is Subgroup ′ G ′ :> isNormal) H}} where
      ...
      isMonoid._`cong-⋆`_ isMonoid:GroupQuot {a₀ = [ a₀ ]} {a₁ = [ a₁ ]} {b₀ = [ b₀ ]} {b₁ = [ b₁ ]} (incl p) (incl q) =
        let P₀ : H (a₁ ⋆ (b₀ ⋆ ◡ b₁) ⋆ ◡ a₁)
            ...
            P₂ = ((a₀ ⋆ ◡ a₁) ⋆ (a₁ ⋆ (b₀ ⋆ ◡ b₁) ⋆ ◡ a₁))  ≣⟨ assoc-l-⋆ ⟩
                (a₀ ⋆ (◡ a₁ ⋆ (a₁ ⋆ (b₀ ⋆ ◡ b₁) ⋆ ◡ a₁)))  ≣⟨ refl `cong-⋆` assoc-r-⋆ ⟩ -- assoc-r-⋆ {- {a = ◡ a₁} -} {- {{TT = make:> {{{!!}}} {{{!!}}}}} -} ⟩
                (a₀ ⋆ (◡ a₁ ⋆ (a₁ ⋆ (b₀ ⋆ ◡ b₁)) ⋆ ◡ a₁))  ≣⟨ {!!} ⟩
